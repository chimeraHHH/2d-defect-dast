...
with open('/root/autodl-tmp/2d-defect-dast/tmd_samples_summary.md', 'w') as f:
    f.write("# VS2/CrS2 TMD 新样本计算结果汇总\n\n")
    f.write("| 编号 | 化学式 | 基底 | 掺杂物 | 缺陷超胞能量 (en2, eV) | 本征超胞能量 (host, eV) | 掺杂物化学势 (mu, eV) | 形成能 (E_form, eV) | 状态 |\n")
    f.write("|---|---|---|---|---|---|---|---|---|\n")
    ...
    for i, row in enumerate(rows):
        ...
        status = "Completed" if eform != "N/A" else "Incomplete/Failed"
        ...
        f.write(f"| {idx} | {row.formula} | {row.host} | {row.dopant} | {en2_str} | {host_str} | {mu_str} | {eform} | {status} |\n")
