...
# We use our successful VS2 and CrS2 hosts, and add 10 new dopants for each (total 20).
# These dopants are well-known to the model (transition metals and alkaline earth metals).
hosts_info = {
    'VS2': {'a': 3.22, 'thickness': 3.2, 'dopants': ['K', 'Mg', 'Ca', 'Sc', 'Ti', 'Cr', 'Mn', 'Fe', 'Cu', 'Zn']},
    'CrS2': {'a': 3.00, 'thickness': 3.1, 'dopants': ['K', 'Mg', 'Ca', 'Sc', 'Ti', 'V', 'Mn', 'Fe', 'Cu', 'Zn']}
}
...
for host, info in hosts_info.items():
    ...
    # Adatom above a transition metal atom (index 0 is a TM atom)
    tm_idx = 0
