...
separation.
This is particularly relevant for defects, whose strain fields and
charge-density perturbations propagate as approximately $1/r^2$ in 2D,
well beyond typical GNN cutoff radii.
...
To enrich the learned atomic representations, we jointly train on four
databases: IMP2D (primary), JARVIS-2D, JARVIS-3D, and a curated DFT-3D
formation energy set.
Each source has an independent readout head while sharing the local and
global encoder layers, preventing label-space contamination while
transferring structural knowledge.
