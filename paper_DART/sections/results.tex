...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/periodic_table_mae.pdf}
\caption{Per-dopant MAE mapped onto the periodic table.
  Each cell shows the element symbol, MAE (eV), and test-sample count.
  Main-group metals (Bi, Pb, Sr) are easiest; early transition metals with
  partially filled $d$-shells (Ta, Mn, Sc) are hardest.
  Grey cells indicate elements not present in the IMP2D test set.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/uq_calibration.pdf}
