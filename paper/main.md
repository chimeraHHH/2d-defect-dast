...
lmax=1) 0.86 eV）以及前述 ALIGNN (0.540 eV)。我们的混合架构以 0.516 eV
全面胜出。进一步通过 5×4 网格扫描（N ∈ {0.5–8}k 训练样本，P ∈ {0.12–1.65}M
...
| **SOTA GNN 对照** | SchNet 0.585 / ViSNet (lmax=1) 0.78 / ours **0.516** / ALIGNN 0.540 | §5.16 |
...
1. **CrystalTransformer 是该数据集上最高效的架构**——以 SchNet 的 1.6×
   ...
   以 ViSNet 的 65% 参数量获得 40% 提升。
2. **ViSNet 等变模型并未带来精度增益**：理论上 lmax=1 的 SO(3) 等变
   架构应能更高效地利用几何信息，但在 IMP2D 这种小数据集上，等变约束
   反而成为优化负担，需要降低学习率 5× 以避免发散。这与 §5.13 的旋转
   不变性测试结论一致：对**标量预测**任务，距离-角度特征已经隐式提供了
   等变性，显式的中间表征等变是冗余的复杂性。
...
13. **SOTA GNN 全面对比确认架构选择**：在同一 leak-free 划分上，SchNet
    (0.46M, 6 层局部) 0.585 / ViSNet (lmax=1, 1.16M, E(3) 等变) 0.78 /
    ALIGNN (4M, 线图) 0.540 / **CrystalTransformer (0.75M, 局部+全局)
    0.516**——本文方案是参数效率与精度的最优点。等变约束在小数据集
    上反而成为优化负担（ViSNet lr=5e-4 第 10 epoch 即 NaN，需要 lr=1e-4
    才稳定），印证了 §5.13 的论断："对标量预测，距离-角度特征已隐式
    具备等变性，显式中间表征等变是冗余复杂性"。
...
方法仍输 2.24×，证明深度学习是必要的；(xi) 等变约束 (E(3) lmax≥1) 在
小数据集 (~10⁴) 标量预测任务上可能成为优化负担——若无明确等变需求
（如力/应力），距离-角度的标量不变特征通常是更鲁棒的选择；
