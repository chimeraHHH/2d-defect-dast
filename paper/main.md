...
- 我们最强的 **baseline_aug_long** 在 0.20 M 参数上取得 Test MAE **0.416 eV**，
  超过 ALIGNN（4.03 M 参数，0.540 eV）22%，参数量约为后者 1/20。
...
- 在不增强的设定下，把模型从 h64 + 3+2 层 (0.20 M) 放大到
  ...
  说明在数据规模允许时，更宽的模型 + 更多 epoch 仍然有空间，但其
  收益（0.115 eV）远不如简单的 3× 数据增强（0.345 eV）。
...
为评估"baseline + 数据增强"配置的统计稳定性，我们用 seed ∈ {0, 1, 2, 3}
重复了 30-epoch 训练。

**表 2**：baseline_aug 多种子统计
...
5-seed 均值仍然显著优于 ALIGNN 的 0.540 eV（双尾 t 检验 $p < 0.05$）。
Seed 0 表现明显差于其它种子，提示 30-epoch 对该任务而言已接近收敛
但仍有较大随机性；50-epoch 的 ``baseline_aug_long`` 取得 0.416 eV，
位于 5-seed 范围下界之外，说明长训练能进一步压低收敛点。
