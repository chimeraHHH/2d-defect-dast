...
0.75 M 参数；配合**严格防泄漏的几何不变性数据增强**（先划分后增强）训练
50 epoch 后，在 IMP2D 测试集（与 ALIGNN 同一 1065 样本测试集）上取得
**MAE = 0.516 eV，RMSE = 1.131 eV**，与 ALIGNN（MAE 0.540, 4.03 M 参数）
精度相当但参数量约为后者 **1/5**。在更紧凑的 0.20 M 参数版本下，同一训练
配方取得 MAE 0.628 eV，仍保持在 ALIGNN ± 0.1 eV 量级。
...
|---|---|---|---|---|
| **baseline_h128_aug_long_safe** (h128, 50 ep, leak-free aug) | 0.747 | **0.516** | 1.131 | 本文最强配置 |
...
- 我们最强的 ``baseline_h128_aug_long_safe`` 在 0.75 M 参数上取得 Test
  MAE **0.516 eV**，相较 ALIGNN（4.03 M, 0.540 eV）参数减少 5.4×、
  误差降低 **4.5%**——并非"碾压"，而是**"以 1/5 参数追平"**。
- 在更紧凑的 h64 (0.20 M) 上，``baseline_aug_long_safe`` 取得 0.628 eV，
  仍在 ALIGNN ± 0.1 eV 量级，对超低算力场景具有吸引力。
...
我们对最强配置 ``baseline_h128_aug_long_safe`` 用 seed ∈ {0, 1, 2, 42}
重复训练以评估随机性。（所有种子结果由 ``scripts/run_queue.sh`` 类似的
``run_honest.sh`` 自动生成。）

**表 2**：``baseline_h128_aug_long_safe`` 多种子统计（各 50 epoch）
...
| 1 | [pending] | – |
| 2 | [pending] | – |
| **mean ± std (4 seeds)** | [pending] | [pending] |

[占位待补：等 4-seed 跑完后填入；预期 mean 约 0.50–0.55 eV，std 约
0.04–0.06 eV。若所有 seed 都低于 ALIGNN 的 0.540，则可宣告"严格优于 ALIGNN"；
若部分 seed 高于 ALIGNN，则只能宣告"与 ALIGNN 在统计意义上相当"。]
