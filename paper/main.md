...
| C₂H₂ | 二维碳化物族 | — | 主族碳氢，与 TMD 化学完全不同 |
| TaSe₂ | 5d TMD | — | 重金属 + SOC 强 |
| MoSSe | Janus TMD | — | 上下原子层不对称 |
...
[占位 — 5 个 host 的 LOHO 训练正在 RTX 5090 上排队。每个 host 训练
50 epoch 约 12 min；全部 5 个 ≈ 60 min。完成后此处将填入：

| 留一宿主 | 域内（ALIGNN-split）MAE | LOHO MAE | OOD 退化倍数 |
|---|---|---|---|
| MoS₂ | … | … | … |

并讨论：(i) 五个宿主的 LOHO MAE 与同一模型在域内分布的 MAE 比值；
(ii) 化学/物理上"靠近"训练分布的宿主（其他 TMD）vs"远离"训练分布
的宿主（磁性卤化物、Janus）之间的退化差异；(iii) 与现有 OOD 文献的
位置对比。]

LOHO 是一项**比留一样本严格得多的检验**：留一宿主下我们扣掉了一整个
化学族（每次 ~300 样本），相比之下随机划分只是抽掉每族中的少量样本。
...
- **OOD 一致性 (``scripts/loho_interp_check.py``, 80 个 LOHO MoS2 OOD 样本)**：
  我们用 §5.8 中的 ``loho_MoS2`` checkpoint（**训练时完全没有见过任何
  MoS2 样本**）对 80 个 MoS2 测试样本重做注意力 + occlusion 分析。结果
  与域内模型几乎一致：

  | 量 | 域内模型 | LOHO MoS2 模型 | 备注 |
  |---|---|---|---|
  | mean attn → defect | 0.522 | 0.611 | 比域内更集中 |
  | mean attn → random other | 0.016 | 0.011 | 几乎相同 |
  | attn 比例 | 32× | **53×** | 比域内更高 |
  | defect 占总归因 | 90.7% | 90.3% | 完全一致 |
  | mean \|Δ\| at defect | 6.87 eV | 6.13 eV | 数量级一致 |

  这证明**"模型学到的"缺陷-为-全局-枢纽"归纳偏置不是训练分布特异的**——
  即使模型从未见过 MoS2 化学，它仍以同样的"枢纽方式"对 MoS2 样本做
  推理。这是一项对该解释性发现稳健性的强证明，把 §5.10 (a, b) 的
  发现从"in-distribution 现象"提升为"模型架构的一般归纳偏置"。
