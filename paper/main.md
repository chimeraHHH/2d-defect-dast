...
**(VI) SOTA 对照与缩放律**：为消除"我们的方法只是赢在精调"的怀疑，
我们在同一 leak-free 划分上系统对比了**经典基线**（LightGBM 1.16 eV）、
**SOTA GNN**（torch_geometric 实现的 SchNet 0.585 eV / ViSNet (E(3) 等变,
lmax=1) 0.78 eV）以及前述 ALIGNN (0.540 eV)。我们的混合架构以 0.516 eV
...
盲目扩大模型反而过拟合。这给出明确的部署/扩展指引：**先扩数据再扩模型**。
...
平台提供了**十二条务实指引**：(i) 先做几何不变性增强（且必须 leak-free）；
