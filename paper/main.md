...
| **OOD 退化谱** | MoS₂ 1.00× / Cr₂I₆ 1.63× / **C₂H₂ 4.65×（R²=0.03 灾难失败）** | §5.8 |
| **注意力指向缺陷** | 平均权重比 **32×**（vs 非缺陷原子）；3 个 OOD 模型仍 26×/53×/685× | §5.10 (a) |
| **occlusion 缺陷归因** | 单原子占总归因 **90.7 ± 13.0%**；3 个 OOD 模型 90.3% / 91.0% / 78.9% | §5.10 (b) |
...
| TaSe₂ | (待) | 5d TMD | WSe₂, MoSe₂ 等 |
| MoSSe | (待) | Janus TMD | WS₂/MoS₂ 等同 Mo + 同元素 |
...
强相关。当训练集已包含同族化学（多个 TMD），模型可"插值"未见过的
TMD（MoS₂ 案例）；当训练集化学结构基本完整（同 d 区块 + 类似配位），
退化温和（Cr₂I₆ 案例）；但当训练集化学结构与目标相去甚远（C₂H₂ 案例
中训练集几乎没有同类），模型外推完全失败。
...
6. **留一宿主验证 (LOHO) 给出更严格的 OOD 评估**：5 个二维材料家族的
   留一退化倍数（MoS₂ 1.00× / Cr₂I₆ 1.63× / **C₂H₂ 4.65×（R²=0.03 灾难
   失败）**等）告诉我们模型在"训练时从未见过的化学族"上的泛化界限随
   化学距离呈非线性恶化；该指标应作为该领域基准的标配。一个特别有用的
   双层失败诊断：**架构归纳偏置（"defect-as-hub"）在所有 OOD 难度下都
   稳定保持，但化学映射在远 OOD 化学族下崩溃**——指向"用元学习/主动
   学习补充少量代表性样本"作为最经济的 OOD 修复路径。
