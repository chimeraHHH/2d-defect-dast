...
2. **TaSe₂ → 1.23×**：5d TMD，与训练集中其他 TMD（含 4d 与 5d 分量）
   部分共享化学键合规律；OOD 退化温和（R² 0.70），符合"近 OOD"预期。
3. **Cr₂I₆ → 1.63×**：磁性 vdW 卤化物——强相关 d-d 交换、自旋 - 轨道
   多电子效应在训练集（主要为 TMD + 主族）中欠表达，模型外推退化明显，
   R² 由 0.95 降到 0.66。
4. **C₂H₂ → 4.65×（catastrophic OOD）**：二维碳氢化学族（仅含 C 和 H）
...
5. **MoSSe**（待完成）：Janus 不对称 TMD，预期 1.0–1.2× 轻度退化。

**4-host 已观察的总体规律**：OOD 退化倍数与训练集中"化学近邻样本数"
负相关——TMD 类（MoS₂ / TaSe₂）≤ 1.23×，磁性 / 异类化学（Cr₂I₆）
1.63×，化学距离极远（C₂H₂）4.65×。**当前 4-host 平均 OOD 退化为
2.13×**，但其中 78% 来自单一灾难案例 C₂H₂；剔除该极端值后剩余 3-host
平均仅 1.29×。这说明对于"训练集中具有同类化学先验"的二维材料家族，
模型外推有充分可信度；但对全新化学族（如纯主族碳氢），需要补充至少
该家族的少量代表性样本才能恢复可用精度。
...
- **OOD 一致性 (``scripts/loho_interp_check.py``)**：
  ...
  | 量 | 域内 | LOHO MoS2 | LOHO TaSe2 | LOHO Cr2I6 | LOHO C2H2 |
  |---|---|---|---|---|---|
  | OOD 退化倍数 | 1.00 (基线) | 1.00× | 1.23× | 1.63× | **4.65×** |
  | mean attn → defect | 0.522 | 0.611 | 0.730 | 0.499 | **0.931** |
  | mean attn → random other | 0.016 | 0.011 | 0.008 | 0.020 | 0.001 |
  | attn 比例 | 32× | 53× | **92×** | 26× | **685×** |
  | defect 占总归因 | 90.7% | 90.3% | 86.8% | 91.0% | **78.9%** |
  | mean \|Δ\| at defect | 6.87 | 6.13 | 5.85 | 7.14 | **1.95** eV |
