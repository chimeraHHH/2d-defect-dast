...
| ViSNet (lmax=1) | 1.16 | E(3) 等变 (4 层, num_heads=4) | 0.78 | 需要 lr=1e-4 + clip=1.0 才稳定（lr=5e-4 第 10 epoch 就发散 NaN）|
...
平台提供了九条务实指引：(i) 先做几何不变性增强（且必须 leak-free）；
