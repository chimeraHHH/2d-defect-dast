...
subset specifically, the per-dopant-corrected MAE is $\sim 5\times$
...
is $\sim 5\times$ the in-distribution test MAE of the scoring
ensemble: a concrete deployment-relevant gap, not a universal
upper bound.
...
rows use \texttt{split\_indices(seed=$N$)} folds of the cleaned
IMP2D, whose seed=42 fold is verified element-wise identical to the
canonical 1065-sample fold (Sec.~\ref{sec:multi}), so all seed=42
rows in this table share one fixed test partition. 4-seed entries
report mean~$\pm$~standard deviation across seeds 42, 0, 1, 2.}
\label{tab:headline}
\begin{tabular}{lrrr}
\toprule
...
\paragraph{Test partitions: seed=42 is exactly the canonical fold.}
The multi-source pipeline uses \texttt{split\_indices(seed=$N$)} to
partition IMP2D, so the test fold differs across seeds 0/1/2. For
seed=42, however, the partition is \emph{element-wise identical} to
the canonical leak-free 1065-sample test fold used by all
single-source baselines, because the leak-free dataset itself was
constructed with \texttt{split\_indices(seed=42)}. We verify this by
comparing the stored DFT target arrays of the saved
\texttt{test\_predictions.npz} across runs
(\texttt{scripts/verify\_v2\_test\_partition.py};
all 1065 labels match in order to $10^{-6}$). The seed=42 comparisons
--- v1 single-source 0.516, v1 multi-source 0.555, \modelname\
0.4929 --- are therefore apples-to-apples on one fixed test
partition; only the 4-seed mean (0.486) additionally averages over
the seed-0/1/2 partitions.
...
MAE is 2.66\,eV (95\% CI $[1.59, 3.97]$), a factor of $\sim 5\times$
the IMP2D test-fold MAE of 0.537\,eV of the v1 scoring ensemble
(Sec.~\ref{sec:prospective}). Any deployment that uses a
...
\begin{enumerate}[leftmargin=20pt, itemsep=2pt]
\item \textbf{Test partitions (largely resolved).} Multi-source
experiments use \texttt{split\_indices(seed=$N$)} random folds. A
post-hoc verification
(\texttt{scripts/verify\_v2\_test\_partition.py},
\texttt{results/v2\_test\_partition\_verification.json}) shows the
seed=42 fold is element-wise identical to the canonical leak-free
1065-sample fold, so the seed=42 single-source vs.\ multi-source
comparison (0.516 vs.\ 0.4929) is exactly apples-to-apples. The
residual caveat is confined to the 4-seed mean (0.486), whose
seed-0/1/2 members are evaluated on different (equally leak-free)
partitions of the same cleaned corpus.
...
OOD subset, the per-dopant-corrected MAE is $\sim 5\times$ the
in-distribution test MAE of the scoring ensemble: a concrete
deployment-relevant gap that
