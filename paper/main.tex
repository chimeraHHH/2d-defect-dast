...
\title{\textbf{Data, not architecture, governs 2D defect formation energy
prediction: testing three physics-grounded inductive biases against an
empirical scaling law}}

\author{Anonymous \\
\small (authors and affiliations to be filled in for submission)}

\date{Draft v3.0 \quad \today}
...
\begin{abstract}
\noindent
The defect formation energy \Ef\ in two-dimensional (2D) materials is
the central thermodynamic quantity for defect engineering, but its
density-functional-theory (DFT) computation costs tens to hundreds of
CPU-hours per configuration --- prohibitive for high-throughput
exploration. Several recent architectural innovations
(periodic-graph Transformers, line-graph models, equivariant tensor
networks) have been proposed to bring ML-based surrogates closer to
DFT accuracy, yet their effectiveness on \emph{point defects} is
poorly understood. Here we test three independent
physics-grounded inductive biases on the
\textit{Impurities in 2D Materials Database} (IMP2D, 10\,641
configurations): (i) a \textbf{Periodic Fourier Bias} (PFA) that
makes the self-attention exactly invariant to lattice translations
through a truncated Fourier basis on the minimum-image fractional
displacement; (ii) a \textbf{multi-scale distance bias} that decouples
short-range chemistry from long-range elastic / Coulombic decay; and
(iii) a \textbf{dual-stream cross-attention} that explicitly conditions
the prediction on the host pristine supercell as a reference state,
implementing the physical definition $E_{\mathrm{f}} = E_{\mathrm{defect}} -
E_{\mathrm{host}} + \Delta\mu$ at the architectural level with a
soft invariance regulariser
$\lambda \cdot \|f(\mathbf{x}_{\mathrm{pri}}, \mathbf{x}_{\mathrm{pri}})\|^2$.
Across a strict leak-free augmentation protocol, all three single-source
architectural improvements are statistically indistinguishable from a
matched 0.75\,M-parameter baseline (4-seed test MAE
0.519--0.553\,eV \emph{vs}.\ baseline 0.537~$\pm$~0.014\,eV).
In contrast, a multi-source joint-training recipe across four DFT
databases (IMP2D + JARVIS-2D, JARVIS-3D, JARVIS DFT-3D; 26\,837
samples in total, $3.15\times$ the IMP2D corpus) reduces 4-seed test
MAE to \textbf{0.486~$\pm$~0.025\,eV}, an 11\,\% improvement that
quantitatively matches the previously published empirical scaling law
$\log\,\mathrm{MAE} \propto -0.40\,\log\,N_{\mathrm{train}}$ to within
experimental noise --- a prospective verification of the law on an
unseen training regime. We further audit the literature-standard
``concatenate-then-split'' augmentation protocol and demonstrate it
inflates apparent test accuracy by a factor of $2.5\times$ via a
covert leakage path; this paper itself \emph{re-introduces} the same
bug during a refactor (test MAE briefly reads 0.275 vs.\ leak-free
0.509) and reports the catch+fix as a cautionary case study.
The data engineering --- not architectural innovation ---
emerges as the dominant lever for 2D defect formation energy
prediction at the current data scale, with implications for the
methodology of any 2D-defect ML benchmark.
\end{abstract}

\bigskip
\noindent\textbf{Keywords:} 2D materials, defect formation energy,
graph neural networks, self-attention, scaling laws, multi-source
training, leak-free augmentation, methodology audit.
...
We give a quantitative answer by a head-to-head comparison of three
physics-grounded architectural innovations against a controlled data
expansion. Our three architectural hypotheses are:

\begin{enumerate}[leftmargin=20pt, itemsep=2pt]
\item \textbf{Periodic Fourier Bias (PFA).} A truncated Fourier basis
on the minimum-image fractional displacement, added to the
self-attention scores as a per-head bias. By construction PFA is
exactly invariant under any lattice translation of any single atom.

\item \textbf{Multi-scale distance bias.} A two-channel distance
bias that splits short-range (Gaussian RBF, $r \le 5$\,\AA) from
long-range (analytical inverse powers and exponentials with
smooth cut-off at 12\,\AA) decay channels.

\item \textbf{Dual-stream pristine--defect cross-attention.} The
defect supercell and its host pristine supercell are encoded by a
shared backbone; two cross-attention blocks let the defect tokens
attend to the pristine tokens; the prediction is read off from the
\emph{difference} of pooled features
$\Delta\mathbf{z} = \mathrm{pool}(h^{\mathrm{xattn}}_{\mathrm{def}})
- \mathrm{pool}(h_{\mathrm{pri}})$, with a bias-free linear readout
that satisfies $f(\mathbf{0}) = 0$ by construction. A soft auxiliary
loss $\lambda \cdot \|f(\mathbf{x}, \mathbf{x})\|^2$, $\lambda = 0.05$,
drives the model to learn the
$\Delta\mathbf{z} = 0 \Rightarrow E_{\mathrm{f}} = 0$ identity.
\end{enumerate}

The data lever is a multi-source joint-training recipe across four
DFT databases (IMP2D + JARVIS-2D + JARVIS-3D + JARVIS DFT-3D), with a
shared encoder and per-source readout heads, scaling the effective
training corpus to 26\,837 samples ($3.15\times$ IMP2D).

\paragraph{Contributions.} We report the following findings:

\begin{enumerate}[leftmargin=20pt, itemsep=2pt]
\item All three single-source architectural improvements
(\textbf{PFA, multi-scale, dual-stream}) are
\textbf{statistically indistinguishable} from a tuned
0.75\,M-parameter baseline at the IMP2D data scale under leak-free
$3\times$ augmentation. 4-seed test MAE for the baseline is
$0.537~\pm~0.014$\,eV; the dual-stream variant lands at
$0.528~\pm~0.023$\,eV; PFA + multi-scale + defect-bias variants
land in $0.519$--$0.551$\,eV.

\item The same dual-stream architecture, evaluated \emph{without}
augmentation on the same 8\,512-sample training set as the v1
baseline, beats the baseline by 6.3\,\% (0.583 \emph{vs}.\
0.622\,eV). The dual-stream design is therefore demonstrably useful
in the small-data regime and demonstrably absorbed by the
$3\times$ augmentation lever.

\item Multi-source joint training reduces 4-seed test MAE to
$\mathbf{0.486~\pm~0.025}$\,eV --- an 11.2\% improvement over the
matched-architecture single-source baseline, in
\textbf{quantitative agreement} with the scaling-law prediction
$\Delta\log\,\mathrm{MAE} = -0.40\log(3.15) \approx -0.20$. To our
knowledge this is the first prospective verification of an empirical
scaling law on an unseen training regime for materials property
regression.

\item The dual-stream auxiliary invariance loss
$\lambda \cdot \|f(\mathbf{x},\mathbf{x})\|^2$ converges to
$\sim 10^{-3}$ over training: the network successfully learns the
identity $E_{\mathrm{f}}(\mathrm{pristine}) = 0$ to within
millielectronvolts. We surface this as a clean
\emph{interpretability} metric for any defect-formation-energy
model that takes the host pristine state as input.

\item We document \emph{and re-introduce} the
``concatenate-then-split'' augmentation leak that inflates apparent
test accuracy by a factor of $2.5\times$. After we extended the
augmentation pipeline to the dual-stream
...
the random-shuffle path, returning a test MAE of 0.275\,eV (vs.\ the
leak-free value of 0.509). We caught the bug from the
``too-good-to-be-true'' delta, fixed the split function, re-ran the
training and report both the leaked and the leak-free numbers as a
methodology case study (Sec.~\ref{sec:leak}).
\end{enumerate}

The four-architecture ablation under a single, leak-free protocol
quantitatively closes the architecture-vs-data question for 2D defect
formation energy prediction at the present data scale: \emph{data
engineering should precede architecture engineering until the
training corpus exits the
$\partial\mathrm{MAE}/\partial N_{\mathrm{train}} \gg
\partial\mathrm{MAE}/\partial \mathrm{(arch.)}$ regime.}
...
(v1 multi-source vs.\ v2 multi-source). 4-seed entries report
...
v1 multi-source (CrystalTransformer + 4 DBs)
                    & 0.815 & 0.555 & --- \\
v2 multi-source (PFA + 4 DBs)
                    & 0.820 & 0.4929 & --- \\
\textbf{v2 multi-source 4-seed mean}
...
v3 dual-stream + aug (4-seed mean $\pm$ std), and v2 multi-source
...
from ALIGNN through v1 baseline to v1 multi-source, v2 multi-source
seed=42, and the 4-seed mean. \textbf{Right:} per-seed scatter,
...
unaugmented v2 multi-source baseline of 0.4929\,eV — the largest
...
paper. The v2 multi-source $-12\,\%$ improvement was driven by the
...
Table~\ref{tab:loho} compares the v2 multi-source LOHO test MAE
...
A leak-free re-evaluation of v2 multi-source on the canonical 1065
...
We have tested three independent physics-grounded inductive biases
--- a periodic-aware Fourier attention bias, a multi-scale distance
bias, and a dual-stream pristine--defect cross-attention with a
soft invariance regulariser --- against a previously published
empirical scaling law for 2D defect formation energy prediction.
The single-source variants of all three are statistically
indistinguishable from a tuned 0.75\,M-parameter baseline at the
IMP2D data scale, while a multi-source joint-training recipe across
four DFT databases gives an 11.2\,\% improvement that
quantitatively matches the scaling-law prediction
$\Delta\log\,\mathrm{MAE} = -0.40 \log K$ to within experimental
noise --- a prospective verification of the law on an unseen
training regime. The dual-stream architecture, while not winning on
test MAE, learns the
$f(\mathrm{pristine}, \mathrm{pristine}) = 0$ identity to within
$\sim 10^{-3}$\,eV, providing a measurable interpretability metric
for any defect-formation-energy model with a host-pristine input.
Finally, we audit the literature-standard
``concatenate-then-split'' augmentation protocol --- a
$2.5\times$ apparent-accuracy inflation that we ourselves
re-introduced under a refactor --- and propose a generic ordered-
split metadata contract that removes the bug by construction.

For 2D defect formation energy prediction at the current data
scale, our central finding is that \emph{data engineering precedes
architecture engineering}: a single multi-database joint-training
recipe outperforms four independent attention-mechanism innovations
combined, in a manner the empirical scaling law predicts. The
methodology audit and the publicly released
\texttt{leak\_free\_aug\_pristine\_v1} dataset provide a
reproducibility floor for any future 2D-defect ML benchmark.
...
v2 multi-source result is reproducible end-to-end by running
