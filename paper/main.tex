...
$E_f^{\mathrm{DFT}} < +1$\,eV, demonstrated on 20 SCF-converged
top-$\mu$ candidates spanning 18 hosts, exceeds the predicted-pool
base rate by $\sim 4\times$ and is the principal computational
saving over random sampling.
...
4.3\,eV) 1.74\,eV --- is real and useful. But within bucket B,
Pearson($\sigma_{\mathrm{cal}}$, $|\mathrm{error}|$) $= -0.288$:
\emph{higher $\sigma$ does not imply higher error inside the OOD
subset.} Use $\sigma_{\mathrm{cal}}$ to reject candidates whose
$\sigma$ exceeds an in-distribution threshold (e.g.\ the 95th
percentile of training-fold $\sigma$); do not use it to rank within
the rejected pool.
