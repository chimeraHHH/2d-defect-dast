...
score on truly unseen chemistry. The OOD MAE on this subset is
$5.5\times$ the in-distribution test MAE, an explicit deployment
ceiling. \emph{Third}, we release the full pipeline, including
...
The OOD MAE on this subset is $5.5\times$ the in-distribution
test MAE, an explicit deployment ceiling.
...
\item \textbf{Validate decision-critical candidates with DFT.} The
per-dopant-corrected MAE on this OOD validation set is 2.66\,eV,
$5.5\times$ the IMP2D test-fold MAE of 0.486\,eV. Any deployment
that uses a single-point \modelname\ prediction without DFT
follow-up therefore implicitly accepts an absolute uncertainty of
several eV. The 125-SCF pipeline released with this paper makes
...
sample size warrants confirmation at larger N. The OOD MAE on
this subset is $5.5\times$ the in-distribution test MAE, an explicit
deployment ceiling that no test-fold benchmark could have surfaced.
